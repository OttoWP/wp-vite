...
import {EmittedAsset, GlobalsOption, OutputChunk} from "rollup";
...
export function wpViteBundler(userOptions: WpViteBundlerOptions, mode: string): Vite.PluginOption {
    ...
    /**
     * Function to gather resource files from entries and group them by extension
     */
    ...
    /**
     * Create the dependency array for a module.
     *
     * @param module
    ...
    const createDependencyArray = (module: Rollup.OutputChunk, isInteractivity: boolean, globals: Rollup.GlobalsOption, externals: string[]): string[] => {
        ...
        // Add WordPress dependencies based on externals/import we've found inside the file
        if (options.wpDeps) {
...
`;
    ...
    /**
     * Set the importedCSS in the vite metadata and collect styles.
     *
     * @param module
     */
    ...
    /**
     * Create the PHP manifest file to replace JSON file.
     *
     * @param manifestPath
     */
        ...
        async generateBundle(bundleOptions: Rollup.NormalizedOutputOptions, bundle: Rollup.OutputBundle) {
            ...
            for (const module of Object.values(bundle)) {
                if (module.type === 'chunk' && module.facadeModuleId) {
                    ...
                    // Create dependencies array for PHP file.
                    const dependencies: string[] = createDependencyArray(module, isInteractivity, globals, externals);

                    createPhpAssetFile(module, bundleOptions, dependencies) // Create the dependency & cache buster php file.
                    setImportedCss(module) // Get proper outDir folder structure for styles and modify ImportedCss metadata.
                    removeEmptyCssComment(module) // Remove empty CSS comments that gets generated by vite.
                }
            }
